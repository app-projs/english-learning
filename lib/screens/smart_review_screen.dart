...
class _SmartReviewScreenState extends State<SmartReviewScreen> {
  ...
  Future<void> _loadDueReviewItems() async {
    ...
    final wrongAnswers = storage.getWrongAnswers();
    ...
    for (var wa in wrongAnswers) {
      items.add({
        'type': '错题消灭',
        'title': wa['question'] ?? '',
        'subTitle': '类别: ${wa['type'] ?? '错题'}',
        'answer': '正确答案: ${wa['correctAnswer'] ?? ''}',
        'example': '你的回答: ${wa['userAnswer'] ?? ''}',
      });
    }
  ...
  void _rateMemory(int intervalDays) {
    AudioService.instance.speak(_dueItems[_currentIndex]['title']!);
    _reviewedCount++;
