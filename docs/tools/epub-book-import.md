...
- 复合书名会保留全集、卷号和斜杠分隔结构，并要求模型分别判断每部作品的通行译名；结构校验失败时会自动审核重译；
- 每本书最多保留 50 个章节；多章节书籍单章最多保留 100 个生成段落，单章节书籍放宽到 200 个；超出部分截断，原始 EPUB 的分段数量不直接决定章节是否丢弃；
- 章节优先按照 EPUB `spine` 顺序读取；
- 对把整本子书打包进单个 XHTML 的合集，会优先依据同页目录锚点或正文中的 `Chapter 1`、`1. The Cyclone` 等标题切分，再应用 50 章和 100 段限制；
- 章节标题优先使用 EPUB `toc.ncx`、同页目录锚点或正文标题；
...
- EPUB 自带封面会提取为书籍目录下的 `cover.jpg/png`，`coverUrl` 指向该文件；没有封面的 EPUB 会使用书名、中文书名和作者生成本地 `cover.jpg`，不会写入空封面路径；
- 过滤标题页、目录页、导航、脚本和样式；
