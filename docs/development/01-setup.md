...
摆脱数据线进行局域网无线调试可极大提高开发效率。

#### 前提条件
- 电脑和手机连接在同一 Wi-Fi 网络下
- 手机开启"开发者模式"和"USB 调试"

#### 方案 A：Android 11+（无需数据线）

1. 手机进入 **设置 > 开发者选项 > 无线调试**，打开开关
2. 点击 **"使用配对码配对设备"**，记录 IP 地址、端口号和配对码
3. 电脑终端执行：
```bash
adb pair [IP地址]:[端口号]
# 例：adb pair 192.168.1.10:37891
```
4. 输入手机显示的配对码
5. 配对成功后，执行：
```bash
...
```
3. 拔掉数据线
4. 查看手机局域网 IP 地址（Wi-Fi 设置中）
5. 连接手机：
```bash
adb connect [手机IP]:5555
# 例：adb connect 192.168.1.5:5555
```

#### 💡 注意事项
- Wi-Fi 重连后 IP 可能变化，建议在路由器设置静态 IP
- 连接断开后执行 `adb disconnect` 再重新 `adb connect`
- 如遇连接问题，检查防火墙是否允许 5555 端口
