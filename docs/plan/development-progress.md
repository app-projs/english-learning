...
**工具维护**：修复 EPUB 导入工具未写入章节 `chineseTitle` 的问题，并补齐书籍元数据字段、封面提取、完整性校验、书名通行译名提示和章节数量限制；同时支持从合集单 XHTML 内的目录锚点或正文标题切分真实章节，修复异常嵌套 XHTML、目录重复章节、`split_000` 标题页误入正文及跳过章节后编号不连续的问题。  
